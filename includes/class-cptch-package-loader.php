	...
	class Cptch_Package_Loader {
		...
		function save_packages( $pakages_dir = '', $remove_package = true, $existed_action = '' ) {
			...
			if ( $remove_package ) {
				...
				if ( ! $this->error ) {
					$packages_message	= sprintf( _n( 'One package has been updated or added to the database.', '%s packages have been updated or added to the database.', $this->result[0], 'captcha-bws' ), $this->result[0] );
					$images_message		= sprintf( _n( 'One image has been updated or added to the database.', '%s images have been updated or added to the database.', $this->result[1], 'captcha-bws' ), $this->result[1] );
